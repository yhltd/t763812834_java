...
//            }
            ...
            if (!authResult.isSuccess()) {
                ...
                String shenfen = (String) session.getAttribute("C");
                if(!shenfen.equals("仓库员")) {
    ...
    @PostMapping("/export")
    public Result<List<Xdmx>> export(@RequestBody ScgdSearchRequest request, HttpSession session) {
        try {
            // 处理日期格式转换：将-替换为/
            if (StringUtils.isNotBlank(request.getStartDate())) {
                request.setStartDate(formatDateWithZeroPadding(request.getStartDate()));
...
//            }
            ...
            if (!authResult.isSuccess()) {
                ...
                result = xdmxService.exportScgdYW(request, fuzeren);
            } else {
                // 执行导出查询
                result = xdmxService.exportScgd(request);
            }

            return Result.success(result);

        } catch (Exception e) {
            log.error("导出下单明细失败", e);
...
//            }
    ...
    public Result rejectOrder(HttpSession session, @RequestBody DetailRequestDTO request) {
        try {
            // 权限检查
            Result<?> authResult = AuthUtil5.checkAdminAuth(session);
            if (!authResult.isSuccess()) {
                return Result.error(authResult.getCode(), authResult.getMessage());
            }
//            Result<?> authResult3 = AuthUtil3.checkAdminAuth(session);
