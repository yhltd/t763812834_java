...
function batchExportToExcel() {
            ...
            getDetailDataForPrint(ddh, function(detailData) {
                if (detailData && detailData.length > 0) {
                    ...
                    if (detailData[0]) {
                        ...
                        mainData.ddh = ddh;
                        // 注入订单号到每条明细中，确保导出时能显示
                        detailData.forEach(item => { item.ddh = ddh; });
                    }
                    resolve({
                        ddh: ddh,
                        mainData: mainData,
    ...
    Promise.all(promises).then(function(allData) {
        ...
        // 按客户名称分组
        var customerGroups = {};
        validData.forEach(function(order) {
            var khmc = order.mainData.khmc || '未知客户';
            if (!customerGroups[khmc]) {
                customerGroups[khmc] = {
                    mainData: order.mainData,
                    allDetails: []
                };
            }
            customerGroups[khmc].allDetails = customerGroups[khmc].allDetails.concat(order.detailData);
        });

        var wb = XLSX.utils.book_new();
        var customerNames = Object.keys(customerGroups);

        customerNames.forEach(function(khmc) {
            var group = customerGroups[khmc];
            var sheetData = prepareGroupOrderData(group.mainData, group.allDetails);
            var ws = XLSX.utils.aoa_to_sheet(sheetData);

            // 设置列宽
            ws['!cols'] = [
                { wch: 10 }, { wch: 20 }, { wch: 25 }, { wch: 8 },
                { wch: 10 }, { wch: 8 }, { wch: 12 }, { wch: 15 }, { wch: 20 }
            ];

            // 设置行高
            var rowHeights = [];
            for (var i = 0; i < sheetData.length; i++) {
                rowHeights[i] = (i === 0) ? { hpx: 40 } : { hpx: 20 };
            }
            ws['!rows'] = rowHeights;
            ...
            // 添加到工作簿，Sheet名称截断防止过长
            XLSX.utils.book_append_sheet(wb, ws, khmc.substring(0, 30));
        });

        var fileName = '批量对账单_' + formatDate(new Date()) + '.xlsx';
        XLSX.writeFile(wb, fileName);

        swal('导出成功', `已将 ${customerNames.length} 个客户的订单导出到不同Sheet中`, 'success');
...
// 新增：准备分组订单的数据模板
function prepareGroupOrderData(mainData, allDetails) {
    var currentDate = new Date();
    var currentMonth = currentDate.getMonth() + 1;
    var currentYear = currentDate.getFullYear();
    var currentDay = String(currentDate.getDate()).padStart(2, '0');

    var currentPeriodAmount = calculateTotal(allDetails);
    var openingAmount = 0;
    ...
    allDetails.forEach(function(item, idx) {
        excelData.push([
            idx + 1,
            item.pm || '',
            item.ggxh || '',
...
function exportPrintDataToExcel(rowData, detailData) {
    ...
    try {
        ...
        // 确保明细中包含订单号
        detailData.forEach(item => { item.ddh = rowData.ddh; });

        var excelData = prepareGroupOrderData(group.mainData, group.allDetails);
        var fileName = '对账单_' + (rowData.khmc || '客户') + '_' + (rowData.ddh || '订单') + '.xlsx';
